export default {
  'title': 'Ember Weekend',
  'description': 'Ember.js is a frontend JavaScript framework that has exciting applications.  In this podcasts we share news, events, and some of our experiences.',
  'link': 'https://emberweekend.com',
  'site_url': 'https://emberweekend.com',
  'image_url': 'https://i.imgur.com/YyAd2Ee.png',
  'managingEditor': 'jonathan.jackson1@me.com (Jonathan Jackson)',
  'webMaster': 'chase@code0100fun.com (Chase McCarthy)',
  'copyright': '2015 Chase McCarthy & Jonathan Jackson',
  'language': 'en-us',
  'categories': ['Emberjs', 'ember', 'ember-cli', 'podcast', 'javascript', 'programming'],
  'pubDate': 'May 23, 2015 11:00:00 EDT',
  'ttl': '60',
  'custom_namespaces': {
    'itunes': 'http://www.itunes.com/dtds/podcast-1.0.dtd'
  },
  'custom_elements': [
    { 'itunes:subtitle': 'Latest Ember.js news, events, and interesting tidbits.' },
    { 'itunes:author': 'Chase McCarthy & Jonathan Jackson' },
    { 'itunes:summary': 'Ember.js is a frontend JavaScript framework that has exciting applications.  In this podcasts we share news, events, and some of our experiences.' },
    { 'itunes:owner': [
      { 'itunes:name': 'Chase McCarthy' },
      { 'itunes:email': 'chase@code0100fun.com' }
    ] },
    { 'itunes:explicit': 'no' },
    { 'itunes:image': {
      '_attr': {
        'href': 'https://i.imgur.com/YyAd2Ee.png'
      }
    } },
    { 'itunes:category': [
      { '_attr': {
      'text': 'Technology'
    } },
    { 'itunes:category': {
      '_attr': {
        'text': 'Tech News'
      }
    } }
    ] }
  ]
};
